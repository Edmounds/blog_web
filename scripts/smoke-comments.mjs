...
try {
  ...
  const first = await createComment(db, rateRequest, { contentId: "blog/first-note", name: "Smoke Rate", content: "first" }, "local-smoke-secret", new Date("2026-07-23T06:00:00.000Z"));
  const second = await createComment(db, rateRequest, { contentId: "blog/first-note", name: "Smoke Rate", content: "second" }, "local-smoke-secret", new Date("2026-07-23T06:00:30.000Z"));
