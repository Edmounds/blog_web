...
const errors = [];
const requiredLocal = [
  "public/images/content/about/profile-8646bdb863b8-w160.avif",
  "public/images/content/about/profile-8646bdb863b8-w160.webp",
  "public/images/content/about/profile-8646bdb863b8-w320.avif",
  "public/images/content/about/profile-8646bdb863b8-w320.webp",
  "public/images/content/about/profile-8646bdb863b8-social.webp",
...
for (const relativePath of requiredLocal) {
  ...
  try {
    ...
    if (!metadata.width || !metadata.height) errors.push(`${relativePath} has no dimensions`);
    if ((await stat(filePath)).size === 0) errors.push(`${relativePath} is empty`);
  } catch (error) {
...
for (const [fallback, asset] of Object.entries(manifest.assets)) {
  ...
  for (const format of ["avif", "webp"]) {
    ...
    for (const variant of variants) {
      ...
      if (!manifestKeys.has(key)) errors.push(`${variant.url} is not manifest-owned`);
      if (variant.ssim < FALLBACK_SSIM) errors.push(`${variant.url} has SSIM ${variant.ssim}`);
      if (variant.ssim < PRIMARY_SSIM) console.warn(`${variant.url} uses relaxed SSIM ${variant.ssim}`);
      if (!Number.isInteger(variant.width) || variant.width <= 0) errors.push(`${variant.url} has invalid width`);
      if (!Number.isInteger(variant.bytes) || variant.bytes <= 0) errors.push(`${variant.url} has invalid byte size`);
    }
  }
}
...
if (process.argv.includes("--remote")) {
  for (const key of manifest.keys) {
    try {
      ...
      if (!/immutable/i.test(response.headers.get("cache-control") ?? "")) errors.push(`${key} is not immutable`);
    } catch (error) {
...
for (const relativePath of contentFiles) {
  ...
  if (!source.includes("profile-8646bdb863b8-w320.webp")) errors.push(`${relativePath} still uses the unoptimized portrait`);
}
