...
const replaceVariants = async ({ sourcePath, outputDirectory, widths, stem, retainSourceWidth = false }) => {
  const asset = await createResponsiveImage({ sourcePath, outputDirectory, widths, stem, retainSourceWidth });
  return asset.variants;
};

const optimizeProfile = async () => {
  const sourcePath = path.join(publicImages, "content/about/profile-8646bdb863b8.png");
  const outputDirectory = path.dirname(sourcePath);
  const variants = await replaceVariants({
    sourcePath,
    outputDirectory,
    widths: [160, 320],
    stem: "profile-8646bdb863b8",
  });
  const socialPath = path.join(outputDirectory, "profile-8646bdb863b8-social.webp");
  await sharp(sourcePath).rotate().resize(1200, 1200, { fit: "cover" }).toColourspace("srgb")
    .webp({ quality: 90, effort: 6, smartSubsample: true })
    .toFile(socialPath);
  return [...variants.map((variant) => variant.filePath), socialPath];
};
