...
test("the primary canvas keeps five ordered routes while deferring non-home sections", () => {
  ...
  assert.match(
    layout,
    /initialIndex === 1[\s\S]*import\("\.\.\/components\/sections\/AboutSection\.astro"\)/,
  );
});
...
test("the loading icon gates initial and Life navigation", () => {
  ...
  assert.doesNotMatch(
    loader,
    /muelsyse-(?:head|arm|torso|coat|legs|rear-hair)/,
  );
  assert.doesNotMatch(
    loader,
    /water-growth-ring|scientific-staff|skeletal-water-arc|botanical-particles|rhine-life-mark/,
  );
  assert.doesNotMatch(
    loader,
    /loader-character-walk|loader-frame-lift|loader-frame-step/,
  );
...
test("navigation labels stay English, Links sits before Life, and Life exposes the four collection routes", () => {
  ...
  assert.match(
    header,
    /const links = \{ label: "Links"[\s\S]*?<div class="life-menu">/,
  );
...
test("the transparent header keeps its side controls visible while auto-hiding desktop navigation", () => {
  ...
  assert.match(
    header,
    /\.site-header\s*\{[^}]*position:\s*sticky;[^}]*background:\s*transparent;/s,
  );
  assert.match(header, /\.site-header__inner\s*\{[^}]*min-height:\s*4\.5rem;/s);
  assert.match(
    header,
    /--desktop-nav-font-size:\s*clamp\(20px,\s*1\.2rem \+ 0\.4vw,\s*24px\);/s,
  );
  assert.match(
    header,
    /--life-submenu-font-size:\s*clamp\(16\.3px,\s*0\.82rem \+ 0\.13vw,\s*20px\);/s,
  );
  assert.match(header, /--life-submenu-color:\s*var\(--text-faint\);/s);
  assert.match(
    header,
    /\.desktop-nav\s*>\s*a,[\s\S]*?\.life-menu__items a\s*\{[^}]*font-family:\s*var\(--font-ui\);[^}]*font-size:\s*var\(--desktop-nav-font-size\);/s,
  );
  assert.match(
    header,
    /\.mobile-menu\s*\{[^}]*background:\s*var\(--canvas\);/s,
  );
  ...
  assert.match(
    header,
    /@media\s*\(max-width:\s*64rem\)[\s\S]*?\.site-header__inner\s*\{[^}]*min-height:\s*4rem;/,
  );
  assert.match(
    header,
    /@media\s*\(max-width:\s*64rem\)[\s\S]*?\.site-header\s*\{[^}]*background:\s*var\(--canvas\);[^}]*border-bottom:\s*1px solid var\(--border-soft\);/s,
  );
  assert.match(
    header,
    /@media\s*\(max-width:\s*64rem\)[\s\S]*?\.site-signature\s*\{[^}]*transform:\s*translateY\(-0\.625rem\);/s,
  );
  assert.match(
    header,
    /@media\s*\(max-width:\s*30rem\)[\s\S]*?\.site-header__tools\s*\{[^}]*gap:\s*0;/,
  );
  assert.match(
    header,
    /@media\s*\(max-width:\s*30rem\)[\s\S]*?\.site-signature\s*\{[^}]*width:\s*min\(100%,\s*8\.75rem\);[^}]*font-size:\s*2\.15rem;/,
  );
  assert.doesNotMatch(header, /backdrop-filter/);
  assert.match(
    header,
    /\.desktop-nav\s*\{[^}]*--desktop-nav-gap:\s*clamp\(0\.75rem,\s*1\.5vw,\s*1\.5rem\);[^}]*gap:\s*var\(--desktop-nav-gap\);/s,
  );
  assert.match(
    header,
    /\.life-menu__items\s*\{[^}]*top:\s*50%;[^}]*left:\s*100%;[^}]*display:\s*flex;[^}]*align-items:\s*center;[^}]*gap:\s*var\(--life-submenu-gap\);[^}]*width:\s*max-content;[^}]*padding-left:\s*var\(--life-submenu-gap\);[^}]*transform:\s*translate\(-0\.25rem,\s*-50%\);[^}]*white-space:\s*nowrap;/s,
  );
  assert.match(
    header,
    /\.life-menu__items\[data-open="true"\][^\{]*\{[^}]*transform:\s*translate\(0,\s*-50%\);/s,
  );
  assert.match(
    header,
    /@media\s*\(min-width:\s*64\.0625rem\)\s*and\s*\(max-width:\s*78rem\)[\s\S]*?\.life-menu__items\s*\{[^}]*top:\s*100%;[^}]*left:\s*0;[^}]*flex-direction:\s*column;[^}]*align-items:\s*flex-start;[^}]*padding-top:\s*var\(--life-submenu-gap\);[^}]*padding-left:\s*0;[^}]*transform:\s*translateY\(-0\.25rem\);/s,
  );
  assert.match(
    header,
    /@media\s*\(min-width:\s*64\.0625rem\)\s*and\s*\(max-width:\s*78rem\)[\s\S]*?\.life-menu__items\[data-open="true"\][^\{]*\{[^}]*transform:\s*translateY\(0\);/s,
  );
...
test("public content pages stay inside the measured header frame", () => {
  ...
  assert.match(
    global,
    /\.content-container\s*\{[^}]*margin-left:\s*var\(--content-frame-inline-start\);[^}]*margin-right:\s*var\(--content-frame-inline-end\);/s,
  );
  assert.match(
    global,
    /--content-mobile-padding:\s*clamp\(1\.5rem,\s*5vw,\s*3rem\);/,
  );
  assert.match(
    global,
    /@media\s*\(max-width:\s*64rem\)[\s\S]*?\.content-container\s*\{[^}]*margin-inline:\s*0;[^}]*padding-left:\s*max\(var\(--content-mobile-padding\),\s*env\(safe-area-inset-left\)\);[^}]*padding-right:\s*max\(var\(--content-mobile-padding\),\s*env\(safe-area-inset-right\)\);/s,
  );
...
test("top-level writing sections use the compact Astro-star archive layout", () => {
  ...
  assert.match(
    content,
    /font-size:\s*clamp\(2\.3rem,\s*2rem \+ 1\.5vw,\s*4\.3rem\)/,
  );
  assert.match(content, /ArchiveActivityTimeline/);
  assert.match(content, /ArchiveTableOfContents/);
  assert.match(
    content,
    /padding-bottom:\s*clamp\(0\.75rem,\s*1\.5vw,\s*1\.25rem\)/,
  );
  ...
  assert.match(
    activity,
    /@media\s*\(max-width:\s*64rem\)[\s\S]*?\.archive-activity__year\s*\{[^}]*display:\s*none;/s,
  );
  assert.doesNotMatch(
    content,
    /View categories|content-listing__categories|Essays and longer-form writing|Short observations and working notes|Selected work and experiments/,
  );
  ...
  assert.match(
    about,
    /<div class="about-shell content-container">[\s\S]*?<article class="about-article"/,
  );
  assert.match(
    about,
    /\.about-article\s*\{[^}]*max-width:\s*52rem;[^}]*margin-inline:\s*auto;/s,
  );
  assert.match(
    about,
    /<header class="about-article__title">[\s\S]*<h1>About<\/h1>/,
  );
  assert.match(
    about,
    /\.about-article__title h1[^}]*font-family:\s*var\(--font-ui\);[^}]*font-size:\s*clamp\(2\.3rem,\s*2rem \+ 1\.5vw,\s*4\.3rem\);/s,
  );
  assert.match(about, /<Content \/>/);
  assert.doesNotMatch(about, /profile\.data\.(?:name|motto|city|portrait)/);
  assert.doesNotMatch(
    about,
    /profile\.(?:story|meta|focusCards|experience|homeFeatured)/,
  );
});
...
test("section descriptions remain available to page metadata", () => {
  ...
  assert.match(
    read("src/pages/art/book/index.astro"),
    /description=\{site\.page\.bookDescription\}/,
  );
  assert.match(
...
test("the site favicon uses the local profile portrait", () => {
  ...
  assert.match(
    layout,
    /rel="icon" type="image\/webp" href="\/images\/content\/about\/profile-8646bdb863b8-w160\.webp\?v=1"/,
  );
  assert.match(
    notFound,
    /rel="icon" type="image\/webp" href="\/images\/content\/about\/profile-8646bdb863b8-w160\.webp\?v=1"/,
...
test("legacy blogs routes permanently redirect and Series routes are gone", () => {
  assert.match(
    read("src/pages/blogs/index.astro"),
    /Astro\.redirect\("\/blog\/", 301\)/,
  );
  assert.match(
    read("src/pages/\[locale\]/blogs/index.astro"),
    /Astro\.redirect\([\s\S]*, 301\)/,
  );
  assert.throws(() => read("src/pages/series/index.astro"));
  assert.throws(() => read("src/pages/series/[slug].astro"));
  assert.match(
    read("src/pages/links/index.astro"),
    /BaseLayout[\s\S]*LinksSection/,
  );
});
...
test("Blog Note and Project content collections and image sync roots are configured", () => {
  ...
  assert.match(
    images,
    /CONTENT_GROUPS\s*=\s*\["blog",\s*"note",\s*"project"\]/,
  );
});
...
test("music administration separates albums and singles while preserving selectable results after saving", () => {
  ...
  assert.match(
    admin,
    /type !== "music"[\s\S]*type === "book" \? "作者" : "补充关键词"/,
  );
  ...
  assert.match(
    admin,
    /\/api\/admin\/art\/items\?type=\$\{selectedType\}\$\{suffix\}`,[\s\S]*cache: "no-store"/,
  );
  assert.match(
    admin,
    /setItems\(ensuredItem \? \[ensuredItem,[\s\S]*item\.id !== ensuredItem\.id/,
  );
  assert.match(
    admin,
    /setItems\(\(current\) => \[data\.item,[\s\S]*item\.id !== data\.item\.id/,
  );
  assert.doesNotMatch(
    admin,
    /setForm\(blankForm\(type\)\); setCandidates\(\[\]\); await loadItems/,
  );
  assert.doesNotMatch(
    search,
    /itunes\.apple\.com|searchAppleMusic|upgradeAppleArtwork/,
  );
  ...
  assert.doesNotMatch(
    read("src/lib/cover-api.ts"),
    /itunes\.apple\.com|ItunesSearchResponse/,
  );
});
...
test("music administration paginates nine candidates without clearing search context", () => {
  ...
  assert.match(
    admin,
    /candidates\.slice\(candidatePageStart, candidatePageStart \+ MUSIC_PAGE_SIZE\)/,
  );
...
test("public music collection adds Chinese weekly and total ranking tabs after albums and singles", () => {
  ...
  assert.match(
    musicRoute,
    /listNeteaseRanking\(db, "weekly"\)[\s\S]*listNeteaseRanking\(db, "total"\)/,
  );
...
test("public art cards bind external cover fallback once for foreground and blurred images", () => {
  ...
  assert.match(
    card,
    /addEventListener\("error", useFallback, \{ once: true \}\)/,
  );
  assert.match(
    card,
...
test("art save operations expose independent progress and nearby live feedback", () => {
  ...
  assert.match(
    admin,
    /disabled=\{!canSave \|\| isSaving \|\| isTranslating \|\| isUploading\}/,
  );
  assert.match(
    admin,
    /setItems\(\(current\) => \[data\.item,[\s\S]*item\.id !== data\.item\.id/,
...
test("only books and movies expose art translation controls or request translated drafts", () => {
  ...
  assert.match(
    admin,
    /TRANSLATED_TYPES\.has\(type\)[\s\S]*aria-label="翻译语言"/,
  );
  assert.match(admin, /TRANSLATED_TYPES\.has\(type\)[\s\S]*生成翻译草稿/);
  assert.doesNotMatch(
    admin,
    /selectCandidate[\s\S]*if \(TRANSLATED_TYPES\.has\(type\)\) await translate/,
  );
  assert.match(admin, /JSON\.stringify\(\{ type, \.\.\.source \}\)/);
  assert.match(
    admin,
    /translations: translationsForType\(form\.type, form\.translations\)/,
  );
...
test("localized RSS routes and the WakaTime proxy support both server-side key names", () => {
  ...
  assert.match(
    read("src/pages/api/wakatime-badge.svg.ts"),
    /resolveWakaTimeApiKey/,
  );
  assert.match(
    read("src/pages/api/wakatime-status.svg.ts"),
    /resolveWakaTimeApiKey/,
  );
...
test("the homepage renders a compact WakaTime label and native GitHub heatmap", () => {
  ...
  assert.match(
    home,
    /profile-card__intro[\s\S]*profile-card__location[\s\S]*profile-metrics/,
  );
...
test("the homepage aligns latest content with the profile and lists projects last", () => {
  ...
  assert.match(
    home,
    /\.latest-content\s*>\s*section:first-child\s*\{[^}]*padding-top:\s*0;/s,
  );
  assert.match(
    home,
    /href=\{localizePath\(`\/\$\{block\.section\}\/`, locale\)\}/,
  );
});
...
test("the homepage keeps the legacy social icons in the requested order", () => {
  ...
  assert.match(
    home,
    /href="https:\/\/steamcommunity\.com\/profiles\/76561198437201442"/,
  );
  assert.match(
    home,
  ...
  assert.match(
    home,
    /aria-label="GitHub"[\s\S]*aria-label="Bilibili"[\s\S]*aria-label="Email"[\s\S]*aria-label="Steam"[\s\S]*aria-label="NetEase Cloud Music"/,
  );
});
...
test("category archive routes are removed in favor of tags", () => {
  ...
  assert.throws(() =>
    read("src/pages/[locale]/[section]-archive/[archiveSlug].astro"),
  );
...
test("Links is a localized standalone friend-link page without comments", () => {
  ...
  assert.match(
    links,
    /Name[\s\S]*Last4ev3r[\s\S]*Desc[\s\S]*Keep it Simple,Stupid/,
  );
  assert.match(links, /Link[\s\S]*https:\/\/blog\.muelsyse\.us/);
  assert.match(
    links,
    /Avatar[\s\S]*https:\/\/blog\.muelsyse\.us\/avatar\.webp/,
  );
  assert.doesNotMatch(
    links,
    /copy\.intro|朋友、协作者和常去的网站|Friends, collaborators, and sites worth visiting/,
  );
  assert.doesNotMatch(
    links,
    /请在评论区留下|Leave your site name[^.]*in the comments/,
  );
  assert.doesNotMatch(links, /CommentsSection|links-page__comments/);
  assert.match(cards, /https:\/\/hanlife02\.com/);
  assert.match(cards, /\/api\/friend-avatar\?v=1&url=/);
  assert.match(cards, /encodeURIComponent\(url\)/);
...
test("Project and every Life route use the dynamic constellation canvas", () => {
  ...
  assert.match(
    background,
    /<canvas[\s\S]*data-background-kind="constellation"/,
  );
  assert.match(
    background,
    /path === "\/project\/" \|\|[\s\S]*?path\.startsWith\("\/project\/"\)/,
  );
  assert.match(
    background,
    /path === "\/art" \|\|[\s\S]*?path\.startsWith\("\/art\/"\)/,
  );
  assert.match(background, /requestAnimationFrame\(tick\)/);
  assert.match(background, /prefers-reduced-motion: reduce/);
  assert.doesNotMatch(
    background,
    /route-background__constellation line|route-background__constellation circle/,
  );
});
...
test("writing detail pages keep interactions while moving the table of contents to the right", () => {
  ...
  assert.match(
    detail,
    /article-page__content[\s\S]*article-prose[\s\S]*article-page__toc/,
  );
  assert.match(
    detail,
    /--article-title-size:\s*clamp\(1\.82rem,\s*1\.52rem \+ 0\.9vw,\s*2\.95rem\)/,
  );
  assert.match(
    detail,
    /\.article-page__header h1[\s\S]*?font-size:\s*var\(--article-title-size\)/,
  );
  assert.match(
    detail,
    /html:lang\(zh-CN\)[\s\S]*?--article-title-size:\s*clamp\(1\.56rem,\s*1\.36rem \+ 0\.6vw,\s*2\.28rem\)/,
  );
  ...
  assert.doesNotMatch(
    detail,
    /article-page__floating-actions|article-page__toc-mobile|data-scroll-top/,
  );
  assert.match(toc, /data-toc-progress/);
  assert.match(toc, /data-toc-link/);
  assert.doesNotMatch(toc, /labels\.introduction|Introduction|slug:\s*""/);
  assert.doesNotMatch(detail, /data-article-introduction|labels\.introduction/);
