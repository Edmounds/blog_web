...
> 写作状态：可拆分、可修改、可直接发布的长稿。平台额度与产品行为查询于 **2026-07-28**。Cloudflare 会调整套餐、限制和产品名称，正式发布前请再次核对文末官方链接。
...
```bash
# HTML：应重新验证
curl -I https://blog.example.com/blog/example/

# 哈希资源：应长期 immutable
curl -I https://blog.example.com/_astro/example.HASH.js

# 部署版本：不应缓存
curl -I https://blog.example.com/version.json

# 动态读取：连续请求观察 HIT/MISS 与 Server-Timing
curl -I 'https://blog.example.com/api/stats?contentId=blog%2Fexample'
curl -I 'https://blog.example.com/api/stats?contentId=blog%2Fexample'
...
浏览器侧还要确认：首屏高优先级图片只有合理数量；长文后续图片在滚动前不下载；图片实际选择的 `currentSrc` 尺寸与容器相称；没有布局跳动；首页空闲预取不会压住当前页；部署变化只刷新一次。
...
| 目标 | 本文位置 | 关键文件或命令 |
| --- | --- | --- |
| Astro SSR | 上篇“四、1” | `astro.config.mjs`、`output: "server"` |
| 创建数据库和存储 | 上篇“四、2” | `wrangler d1 create`、`wrangler r2 bucket create` |
| 主 Worker bindings | 上篇“四、3” | `wrangler.astro.jsonc` |
| 入口 Worker 与 Service Binding | 上篇“四、4” | `wrangler.entry.jsonc`、`env.ORIGIN.fetch()` |
| Access 与 Secret | 上篇“四、5” | `wrangler secret put`、Access Application |
| 部署顺序 | 上篇“四、6” | 应用 Worker → 入口 Worker |
| CI/CD | 上篇“四、7” | `.github/workflows/deploy.yml` |
| 优选 DNS | 下篇“三” | `<preferred-cname.example>` |
| 网络测试 | 下篇“四” | `dig`、`curl -w`、DevTools、WebPageTest |
| 缓存规则 | 下篇“五” | `_headers`、Cache API、`version.json` |
| 响应式图片 | 下篇“六” | 640/1280/1920、AVIF/WebP、`srcset` |
| SPA 预取 | 下篇“八” | 当前页 → 意图 → 相邻页 → 剩余页 |

# 上线检查清单

## 架构与安全

- [ ] 自定义域名路由只由入口 Worker 持有，没有两个 Worker 同时声明同一路由。
- [ ] Service Binding 配置在入口 Worker，目标名称与 Astro Worker 部署名称一致。
- [ ] 先部署 Astro Worker，再部署入口 Worker。
- [ ] `/admin/*` 和 `/api/admin/*` 同时受 Cloudflare Access 保护。
- [ ] 应用内部验证 Access JWT 的签名、Issuer、Audience 和有效期。
- [ ] Secret 只在 Cloudflare/GitHub Secret 中，不在仓库和构建产物中。
- [ ] 写接口验证同源、鉴权与输入，并返回 `Cache-Control: no-store`。

## 缓存与内容

- [ ] 带哈希的 CSS/JS/字体和内容寻址图片使用一年 `immutable`。
- [ ] HTML 使用 `no-cache` 或等价的重新验证策略，没有缓存一年。
- [ ] `version.json` 使用 `no-store`，部署变化最多自动刷新一次。
- [ ] 评论和统计只做短边缘缓存，写请求不缓存。
- [ ] GitHub/WakaTime 等外部数据 TTL 与更新频率相称。

## 图片与前端

- [ ] 正文图片提供 AVIF/WebP、多尺寸和原图或兼容格式回退。
- [ ] 图片有固有宽高，首要图片高优先级，其余图片懒加载。
- [ ] 只在页面确实使用 R2 图片域名时 `preconnect`。
- [ ] 非首屏组件接近视口或空闲时加载，失败时有稳定占位和反馈。
- [ ] SPA 不会一次预取所有路由，优先响应悬停、聚焦和触摸意图。
- [ ] 加载遮罩不等待整个 `window.load`，并有最短时间与总超时兜底。

## 国内网络验证

- [ ] 移动、联通、电信分别测试，不用单一家庭宽带代替结论。
- [ ] IPv4 和 IPv6 分开测试。
- [ ] 至少覆盖两个地区和晚高峰。
- [ ] 同时记录 DNS、TCP、TLS、TTFB、总时间、LCP 和 CLS。
- [ ] 不把单次 ping 当作最终结论。
- [ ] 证书仍覆盖自有域名，响应可确认命中自己的入口 Worker。
- [ ] 第三方优选服务失效时，可以在一个 DNS TTL 周期内回退默认入口。

## 成本与可观测性

- [ ] 在 Cloudflare Dashboard 观察 Workers 每日动态请求。
- [ ] 观察 D1 rows read/rows written，常用查询有合适索引。
- [ ] 观察 R2 存储、Class A、Class B，而不只看出口流量。
- [ ] 观察 Images 唯一转换数量，避免参数组合无限增长。
- [ ] 对域名、第三方服务、外部 API 和超额使用单独做预算。
