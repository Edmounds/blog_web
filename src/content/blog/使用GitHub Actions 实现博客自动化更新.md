...
写作端不需要理解构建端的任何细节。这是整套流程里我最满意的一点：写文章的时候，我就只是在一个普通的 Obsidian 库里码字。

## Obsidian 配置

需要两个插件。

第一个是 obsidian-git。它在 Obsidian 里提供提交和推送。

第二个是 obsidian-image-auto-upload-plugin，配合图床PicGo使用。往文章里粘贴图片时，它把图片直接上传到我的 R2 ，Markdown 里落下的是一个在线 URL。仓库从头到尾不出现图片二进制，构建流程后续还会基于这个 URL 自动生成 AVIF/WebP 的多宽度版本，写作时不用管。
...
真正的Pipeline藏在 `npm run deploy` 里，展开是这几步：
...
这套流程用下来最大的变化是写作和发布彻底解耦了。以前发文章是一件事，要处理图片、想 URL、部署、检查；现在它只是一次提交，发布成本低到可以忽略，写的频率反而高了。

如果你想抄这套流程，按需求裁剪：不需要多语言，去掉翻译那步；图片不多，本地图片加 git LFS 也能接受；连动态功能都没有的纯静态站，`npm run deploy` 换成任何静态托管的部署命令都成立。核心就一句话：让仓库成为唯一事实来源，让 CI 做所有重复劳动。
