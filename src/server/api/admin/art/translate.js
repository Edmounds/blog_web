import { ART_TRANSLATED_TYPES, error, json, normalizeArtType, readJson, requireSameOriginJson } from "../../../art.js";
...
export async function onRequestPost({ env, request }) {
  try {
    ...
    if (!title || !creator) return error(400, "INVALID_TRANSLATION_SOURCE", "简中标题和作者不能为空。");

    const translations = {};
    const warnings = [];
    for (const [locale, targetLang] of Object.entries(TARGETS)) {
      try {
        const result = await translateFields(env, { title, creator, extra }, targetLang);
...
async function translateFields(env, fields, targetLang) {
  ...
  try {
    ...
    try { return parseFields(translated); } catch { throw primaryError; }
  }
}
...
async function googleTranslate(value, targetLang) {
  const body = new URLSearchParams({ client: "gtx", sl: "zh-CN", tl: targetLang.toLowerCase(), dt: "t", q: value });
  const response = await fetch("https://translate.googleapis.com/translate_a/single", {
    method: "POST", headers: { "content-type": "application/x-www-form-urlencoded;charset=UTF-8" }, body, signal: AbortSignal.timeout(15_000),
  });
  ...
  if (!translated.trim()) throw new Error("Google Translate response was empty.");
  return translated;
}
...
function text(value, limit, allowEmpty = false) {
  ...
  if ((!result && !allowEmpty) || Array.from(result).length > limit) return undefined;
  return result;
}
